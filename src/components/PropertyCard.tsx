...
const PropertyCard = ({
  ...
  return (
    <div className="relative">
      {unavailable && (
        <span className="font-rubik text-3xl absolute top-40 left-10">UNAVAILABLE</span>
      )}
      <div
        className={`bg-white p-3 shadow-sm max-w-lg flex flex-col w-[320px] ${
          unavailable ? "opacity-50 pointer-events-none" : "opacity-100"
        }`}
      >
        <img
          src={image?.url}
          className="mb-3 h-[300px] w-[300px] object-cover"
        />
        ...
        <div
          className={`${
            showMore ? showStyles : hideStyles
          } flex flex-col gap-2`}
        >
